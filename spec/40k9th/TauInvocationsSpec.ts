...
describe("Create40kRoster", function() {
  it("loads test/40k9th/Tau Invocations.rosz", async function() {
    const doc = await readZippedRosterFile('test/40k9th/Tau Invocations.rosz');
          ...
          jasmine.objectContaining({
            ...
            '_units': [
              jasmine.objectContaining({
                ...
                '_modelList': [
                  "Aun'Va",
                  "2x Ethereal Guard (Honour Blade)",
                  "Unit Upgrades (1. Storm of Fire, 2. Sense of Stone, 3. Zephyr's Grace)"
                ],
                '_weapons': [
                  jasmine.objectContaining({'_name': "Honour Blade"}),
                ],
                '_rules': mapWithKeys(["Inspired to Greatness"]),
                '_abilities': {
                  "Abilities": mapWithKeys(["Leadership Caste (Aun'va)", "Paradox of Duality", "Serene Unifier (Aun'va)", "Supreme Loyalty (Aura)"]),
